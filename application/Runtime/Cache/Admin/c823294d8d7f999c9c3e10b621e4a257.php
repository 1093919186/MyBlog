...
<div class="container" id="content">
    <div class="row">
        ...
        <div class="col-md-9 col-sm-7">
            ...
            <form action="/myblog/admin.php/ChangeArt/update" method="post" enctype="multipart/form-data">
                <div class="form-group">
                    <label>Title</label>
                    <input class="form-control" value="<?php echo ($art["title"]); ?>" style="font-family: '微软雅黑', '黑体'" name="title">
                </div>
                <div class="form-group">
                    <label>Writer</label>
                    <input class="form-control" value="<?php echo ($art["writer"]); ?>" style="font-family: '微软雅黑', '黑体'" name="writer">
                </div>
                <div class="form-group">
                    <label>Header</label>
                    <textarea class="form-control" rows="3" style="font-family: '微软雅黑', '黑体'" name="header"><?php echo ($art["header"]); ?></textarea>
                </div>
                <div class="form-group">
                    <label>Icon</label>
                ...
                <div class="form-group">
                    <label>From</label>
                    <input class="form-control" value="<?php echo ($art["origin"]); ?>" style="font-family: '微软雅黑', '黑体'" name="origin">
                </div>
                <div class="form-group">
                    <label>Link</label>
                    <input class="form-control" value="<?php echo ($art["link"]); ?>" style="font-family: '微软雅黑', '黑体'" name="link">
                </div>
                <select class="form-control" style="font-family: '微软雅黑', '黑体'" name="type">
                    <?php if($reprint == 1): ?><option selected="selected">转载</option>
                        <?php if(is_array($cate)): foreach($cate as $key=>$v): ?><option><?php echo ($v["catename"]); ?></option><?php endforeach; endif; ?>
                        <?php else: ?>
                        <option>转载</option>
                        <?php if(is_array($cate)): foreach($cate as $key=>$v): if($v["cateid"] == $ty): ?><option selected="selected"><?php echo ($v["catename"]); ?></option>
                                <?php else: ?>
                                <option><?php echo ($v["catename"]); ?></option><?php endif; endforeach; endif; endif; ?>
                </select>

                <input class="form-control" value="<?php echo ($art["articleid"]); ?>" type="hidden" name="articleId">
